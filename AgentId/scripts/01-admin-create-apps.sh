...
if [ -z "$APP_PREFIX" ]; then
    ...
    echo "The prefix is used for the app registration name (e.g., 'myagent' creates 'myagent-Agent')."
    echo "This should match the azd environment name the developer will use."
    echo ""
    read -p "Enter app name prefix (e.g., myagent, onedriveagent15): " APP_PREFIX
...
echo "  3. Microsoft Graph delegated permissions"
...
echo ""
echo -e "${BLUE}Step 3/6: Adding Microsoft Graph Permissions...${NC}"
...
echo "AGENT_IDENTITY_CLIENT_ID=$APP_ID"
echo "AGENT_IDENTITY_CLIENT_SECRET=$SECRET"
echo ""
echo "# For Bot Service / Teams:"
echo "BOT_MICROSOFT_APP_ID=$APP_ID"
...
echo "  3. Then run: bash scripts/03-admin-create-fic.sh"
echo ""
...
TENANT_ID=$TENANT_ID
AGENT_IDENTITY_CLIENT_ID=$APP_ID
AGENT_IDENTITY_CLIENT_SECRET=$SECRET

# For Bot Service / Teams
BOT_MICROSOFT_APP_ID=$APP_ID
