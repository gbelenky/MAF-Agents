...
# Run: AFTER Entra Admin completes Step 3 (Bot OAuth)
#
# Output: teams-manifest/{AgentName}.zip → upload to Teams Admin Center
#
...
if command -v azd &> /dev/null; then
    ...
    if [ -z "$BOT_MICROSOFT_APP_ID" ]; then
        # Use BOT_CLIENT_ID (the bot uses the same app registration)
        BOT_MICROSOFT_APP_ID=$(azd env get-value BOT_CLIENT_ID 2>&1 | grep -v "^ERROR\|^WARNING\|update\|winget\|Winget" | head -1 || echo "")
    fi
...
if [ -z "$BOT_MICROSOFT_APP_ID" ] || [ -z "$AZD_ENV_NAME" ]; then
    ...
    if [ -n "$ADMIN_OUTPUT_FILE" ]; then
        ...
        if [ -z "$BOT_MICROSOFT_APP_ID" ]; then
            FILE_APP_ID=$(grep "^BOT_CLIENT_ID=" "$ADMIN_OUTPUT_FILE" | cut -d'=' -f2)
...
if [ -z "$AGENT_NAME" ] && [ -n "$AZD_ENV_NAME" ]; then
    # Capitalize first letter of each word in env name (e.g., fabricagent1 -> FabricAgent1)
    AGENT_NAME=$(echo "$AZD_ENV_NAME" | sed -E 's/(^|-)([a-z])/\U\2/g' | sed 's/-//g')
    echo "Using agent name from azd environment: $AGENT_NAME"
fi
AGENT_NAME="${AGENT_NAME:-Fabric Data Agent}"
AGENT_NAME_FULL="${AGENT_NAME_FULL:-$AGENT_NAME - AI Data Assistant}"
